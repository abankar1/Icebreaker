"""Module for interfacing with watsonx.ai LLMs."""
...
def create_watsonx_embedding() -> WatsonxEmbeddings:
    """Creates a watsonx.ai Embedding model for vector representation.
...
def create_watsonx_llm(
    ...
    """Creates a watsonx.ai LLM for generating responses.
...
def change_llm_model(new_model_id: str) -> None:
    ...
    Args:
        new_model_id: The new model ID to use for the LLM (e.g., "meta-llama/llama-3-3-70b-instruct").
    """
    logger.info(f"Changing LLM model from {config.LLM_MODEL_ID} to {new_model_id}")
