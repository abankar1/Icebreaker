...
def _clean_profile_data(data: Dict[str, Any]) -> Dict[str, Any]:
    """Removes empty values and unwanted fields from raw profile data."""
    data = {
        k: v
...
def extract_linkedin_profile(
    ...
    try:
        if mock:
            logger.info("Using bundled mock profile data...")
            with open(config.MOCK_DATA_PATH) as f:
                return _clean_profile_data(json.load(f))
        else:
        ...
        if response.status_code == 200:
            try:
                ...
                data = _clean_profile_data(response.json())
                return data
